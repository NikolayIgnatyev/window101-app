...
// const closeBtnPopupProductSimple = document.querySelector('.popup-product-simple__сlose')
// const popupProductSimple = document.querySelector('popup-product-simple')
// closeBtnPopupProductSimple.addEventListener('click', () =>{
//   popupClose(popupProductSimple);
// })
...
let activeSwiperStory;
let activeSwiperStoryDelay;
let prevIndex = -1;
function getActiveSwiperStory(swiper) {
  swiper.slides.forEach(slide => {
    if (slide.classList.contains('swiper-slide-active')) {
      if (activeSwiperStory != undefined) {
        // блокировка прошлого слайда
        mutedOtherVideos(activeSwiperStory, true);
        activeSwiperStory.autoplay.stop();
        activeSwiperStory.disable();
      }
      const activeSwiperEl = slide.querySelector('.swiper');
      activeSwiperStory = activeSwiperEl.swiper;
      activeSwiperStory.enable();
      activeSwiperStory.autoplay.start();
    }
  });
}
function setBgImg(swiperStory) {
  let slideImg;
  swiperStory.slides.forEach(slide => {
    if (slide.classList.contains('swiper-slide-active')) {
      // можно заменить посиком по индексу
      slideImg = slide.querySelector("img");
      if (slideImg == null) {
        updateOpenedSlide();
        slideImg = swiperStoryboardOpenedSlide.querySelector("img"); // для видосов
      }
    }
  });
  popupStoryes.style.background = "url(" + slideImg.src + ") no-repeat center center / cover";
}
function updateOpenedSlide() {
  swiperStoryboardOpenedSlide = popupLinks[swiperStory.activeIndex];
}
function setDelay() {
  getActiveSwiperStory(swiperStory);
  let videoDelay = 0;
  activeSwiperStoryDelay = 0;
  ...
  on: {
    ...
    autoplayTimeLeft(s, time, progress) {
      s.pagination.bullets.forEach(bullet => {
        if (bullet.classList.contains("swiper-pagination-bullet-active") && popupStoryesOpen) {
          document.body.style.setProperty('--width-active-bullet', 100 - progress * 100 + '%');
        }
      });
    }
  }
...
const tippyBtn = document.querySelector('.tippy-btn');
const tippyContent = tippyBtn.querySelector('.popup-product-simple__info');
(0,tippy_js__WEBPACK_IMPORTED_MODULE_4__["default"])(tippyBtn, {
